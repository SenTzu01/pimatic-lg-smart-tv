...
class WebOSDevice extends events.EventEmitter {
		...
		return new Promise(( resolve, reject ) => {
			...
			this._ws
				.once('open', () => {
				    ...
				    let handshake = require('./hello.json');
	...
	*/
	...
	createAlert ( message, title ) {
		return this._req("ssap://system.notifications/createAlert", {
			"title": title,
			"message": message,
			"modal": true,
			"type": "confirm",
	...
	getActiveChannel() {
		return this._req('ssap://tv/getCurrentChannel')
			.then( result => {
				return result;
			});		
	}
	...
	launchApp( appId ) {
		return this._req('ssap://system.launcher/launch', {
			id: appId
		});
	}
	
	closeApp( appId ) {
		return this._req('ssap://system.launcher/close', {
			id: appId
		});
	}
	...
	launchBrowser( uri ) {
		...
		return this._req('ssap://system.launcher/open', {
			target: url
		});
	}
	...
	getApps() {
		return this._req('ssap://com.webos.applicationManager/listLaunchPoints')
			.then( result => {
	...
	getActiveApp() {
		return this._req('ssap://com.webos.applicationManager/getForegroundAppInfo')
			.then( result => {
				return result;
			});		
	}
	...
	getSoftwareInfo() {
		return this._req('ssap://com.webos.service.update/getCurrentSWInformation')
			.then( result => {
				return result;
			});		
	...
	getPowerState() {
		return this._req('ssap://com.webos.service.tvpower/power/getPowerState')
			.then( result => {
				return result;
			});		
