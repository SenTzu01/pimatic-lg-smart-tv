module.exports = (env) ->
  ...
  class LgSmartTvPowerSwitch extends env.devices.PowerSwitch
    ...
    _checkStatus: () ->
      ...
      remote.connectAsync({ address: @tvIp, key: @key }).then( () =>
    ...
    _getCurrentApp: (remote) =>
      remote.getAppAsync().then( (app) =>
        @emit('currentApp', @tvIp, app) if app?
        return Promise.resolve()
      )
    
    _getCurrentChannel: (remote) =>
      remote.getChannelAsync().then( (channel) =>
        @emit('currentChannel', @tvIp, channel) if channel?
        return Promise.resolve()
      )
    
    _getCurrentInput: (remote) =>
